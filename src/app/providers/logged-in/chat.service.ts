...
export class ChatService {
  ...
  list(page: number = 1, filters: any): Observable<any> {
    let url = this._endpoint + '?page=' + page + '&expand=staff,candidate,candidate.store,staffUnreadCount,recentMessage';
  ...
  view(chat_uuid: string): Observable<any> {
    let url = this._endpoint + '/' + chat_uuid + "?expand=staff,candidate,candidate.store,staffUnreadCount"
    return this._authhttp.get(url);
  }
